...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return (self.startingPosition,corners)#将位置和四个角落是否被访问作为状态
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        if state[1][0] and state[1][1] and state[1][2] and state[1][3]:#四个顶点均访问过则完成
            return True
        else:
            return False
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if list[0]==False:
        ...
        if S<mid:
            S=mid
    if list[1] == False:
        mid = (abs(xy[0] - corners[1][0]) + abs(xy[1] - corners[1][1]))
        if S < mid:
            S = mid
    if list[2] == False:
        mid = (abs(xy[0] - corners[2][0]) + abs(xy[1] - corners[2][1]))
        if S < mid:
            S = mid
    if list[3] == False:
        mid = (abs(xy[0] - corners[3][0]) + abs(xy[1] - corners[3][1]))
        if S < mid:
            S = mid#将四个角落距离当前位置的曼哈顿距离最大值作为启发式函数的值
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if count==0:
        ...
        D = abs(min_loc[0] - loc[0]) + abs(min_loc[1] - loc[1])+abs(min_loc[0] - max_loc[0]) + abs(min_loc[1] - max_loc[1])
